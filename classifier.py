...
class FaceDetector(object):
    ...
    def detect(self, image):
        ...
        smaller then the template size. For each scale, HOG features for all patches are computed. The trained SVM
        ...
        #for Img in image:
        while (image.shape[0] >= self.template_size) and (image.shape[1] >= self.template_size):
            for y in range(0, image.shape[0], self.step_size):
                for x in range(0, image.shape[1], self.step_size):
                    sample = image[y:y + self.template_size, x:x + self.template_size]
                    if sample.shape[0] == self.template_size and sample.shape[1] == self.template_size:
                        sample = extract_hog_features(sample, self.hog_config, visualize=False,
                                                       feature_vector=True).reshape(1,-1)
                        result = self.classifier.decision_function(sample)[0]
                        if result > self.confidence_threshold:
                            boxes.append([int(x/(self.pyramid_scale**scale_times)),
                                          int(y/(self.pyramid_scale**scale_times)),
                                          int((x+self.template_size)/(self.pyramid_scale**scale_times)),
                                          int((y+self.template_size)/(self.pyramid_scale**scale_times))])
                            confidences.append(result)
            image = rescale(image, self.pyramid_scale, multichannel=True)
            scale_times += 1

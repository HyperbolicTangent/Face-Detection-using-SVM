...
def get_positive_features(files, template_size, hog_config):
    ...
    features, hog_image = extract_hog_features(image, hog_config, visualize=True)
    ...
    for i in files:
        ...
        image = read_file(i)
        image = resize(image, output_shape=(template_size, template_size))
        samples = extract_hog_features(image, hog_config, visualize=False, feature_vector=True).tolist()
        features.append(samples)

    features = np.array(features).reshape(s, -1)
...
def get_negative_features(files, template_size, hog_config, scale, samples_per_scale):
    ...
    for i in files:
        ...
        while (np.size(Img, 0) >= template_size) and (np.size(Img, 1) >= template_size):
            sample = image.extract_patches_2d(Img, patch_size=(template_size, template_size), max_patches=samples_per_scale, random_state=1)
            for j in sample:
                num_sample += 1
                samples = extract_hog_features(j, hog_config, visualize=False, feature_vector=True).tolist()
                features.append(samples)
            Img = rescale(Img, scale, multichannel=True)
    logging.debug(f"Feature extraction took {int(time.time() - start_time)} seconds.")
    features = np.array(features).reshape(num_sample, -1)
